...
test("Anthropic Conversation Agent executes a clock tool and automatically resumes to a final answer", async () => {
  ...
  try {
    ...
    assert.equal(reply.evidence.action_ids.length, 2);
    assert.deepEqual(progress, ["running", "succeeded", "running", "failed"]);
    ...
    assert.match(String(unavailableBlock?.content || ""), /workspace_unavailable/);
    ...
    await assert.rejects(
      ...
      (error: unknown) => (error as { code?: string }).code === "conversation_tool_round_limit",
    );
    assert.equal(listConversationActions(sessionId).filter((action) => action.tool_call_id.startsWith("loop_")).length, 8);
